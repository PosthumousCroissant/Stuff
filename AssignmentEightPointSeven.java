...
public class AssignmentEightPointSeven {
    ...
    public void removeOwner() {
        String name;
        do {
            name = input.readDogText("Enter the name of the user");
        ...
        for (Owner owner : listOfOwners) {
            if (owner.getName().equalsIgnoreCase(name.trim())) {
                searchedOwner = owner;
            }
        }
        if (searchedOwner != null) {
            removeOwnersDogs(searchedOwner);
            listOfOwners.remove(searchedOwner);
        } else if (searchedOwner == null) {
            System.out.println("Error: no such owner");
        }
    }

    private void removeOwnersDogs(Owner owner) {
        ArrayList<Dog> tmpList = new ArrayList<>();
        for (Dog dog : dogList) {
